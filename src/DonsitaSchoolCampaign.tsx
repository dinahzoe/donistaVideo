...
export const MyVideo: React.FC = () => {
    return (
        <AbsoluteFill style={{ backgroundColor: 'white' }}>
            {/* Hier kommt dein Child-Element rein */}
            <Child frame={0} fps={30} />
        </AbsoluteFill>
    );
};
...
const armAngle = Math.sin((frame / fps) * Math.PI * waveSpeed) * armAmplitude;
    ...
    return (
        <g transform={`translate(540, ${1200 + bobY})`}> 
...
const Child = ({ frame, fps }: { frame: number; fps: number }) => {
    const bobY = interpolate(Math.sin((frame / fps) * 2 * Math.PI), [-1, 1], [-8, 8]);
    const bounce = interpolate(Math.sin((frame / fps) * 1.5 * Math.PI), [-1, 1], [0.95, 1.05]);

    return (
        <g transform={`translate(540, ${1200 + bobY}) scale(${bounce})`} fontSize={400}>
            <text x={-250} y={0} textAnchor="middle">🧒🏼</text> {/* Junge */}
            <text x={250} y={0} textAnchor="middle">👧🏽</text>  {/* Mädchen mit langen Haaren */}
        </g>
    );
};
